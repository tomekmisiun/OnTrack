...
def get_day(date_str):
    ...
    meals = MealPlan.query.filter_by(date=day).order_by(MealPlan.position).all()
...
def get_range(start, end):
    ...
    for meal in meals:
        ...
        if key not in result:
            result[key] = []
        result[key].append(meal.to_dict())
...
def add_meal():
    ...
    if not Recipe.query.get(data['recipe_id']):
        return jsonify({'error': 'Przepis nie istnieje'}), 404

    existing = MealPlan.query.filter_by(date=day, position=data['position']).first()
    if existing:
        return jsonify({'error': f'Pozycja {data["position"]} w tym dniu jest już zajęta'}), 409

    meal = MealPlan(date=day, position=data['position'], recipe_id=data['recipe_id'])
    db.session.add(meal)
    db.session.commit()
...
def copy_range():
    ...
    added = 0
    for meal in meals:
        delta = meal.date - source_start
        new_date = target_start + delta

        existing = MealPlan.query.filter_by(date=new_date, position=meal.position).first()
        if not existing:
