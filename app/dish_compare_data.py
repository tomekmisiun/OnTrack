"""Load and build dish-compare data from app/data/dish_compare/."""
...
DISH_COMPARE_DIR = Path(__file__).parent / "data" / "dish_compare"
...
def load_products_seed(lang: str) -> dict[str, dict]:
    lang = lang if lang in SUPPORTED_LANGS else "pl"
    products = _read_json(DATA_DIR / f"products_seed_{lang}.json")
    return {p["name"].lower(): p for p in products}


def load_catalog(lang: str) -> dict[str, dict]:
    """Manual price catalog for dish-compare (EN). Keys are catalog_id."""
    lang = lang if lang in SUPPORTED_LANGS else "pl"
...
def ingredient_cost(product: dict, weight: float) -> float:
...
def validate_manifest_consistency() -> None:
    ...
    for dish_id in sorted(all_dish_ids):
        ...
        if dish_id in dish_ids_en:
            for item in dish.get("ingredients_en") or []:
                catalog_id = item.get("catalog_id")
                if not catalog_id:
                    errors.append(f"dish '{dish_id}': ingredients_en entry missing catalog_id")
                elif catalog_id not in catalog:
                    errors.append(
                        f"dish '{dish_id}': catalog_id '{catalog_id}' not in catalog/en.json"
                    )
...
def build_diy_for_dish_pl(dish: dict, products_by_name: dict[str, dict]) -> dict:
    ingredients_raw = dish["ingredients_pl"]
    ingredients = []
    total = 0.0
    ...
    for item in ingredients_raw:
        ...
        if not product:
            raise ValueError(
                f"dish '{dish['id']}' (pl): product '{item['product_name']}' not in products_seed_pl.json"
            )
        weight = float(item["weight"])
        cost = ingredient_cost(product, weight)
        total += cost
        ingredients.append({
    ...
    return {
        "id": dish["id"],
        "name": dish["name_pl"],
        "portion_note": dish["portion_note_pl"],
        "diy_cost": round(total, 2),
        "ingredients": ingredients,
    }


def build_diy_for_dish_en(dish: dict, catalog: dict[str, dict]) -> dict:
    ingredients_raw = dish["ingredients_en"]
    ingredients = []
    total = 0.0
    ...
    for item in ingredients_raw:
        ...
        if not entry:
            raise ValueError(
                f"dish '{dish['id']}' (en): catalog_id '{catalog_id}' not in catalog/en.json"
            )
        weight = float(item["weight"])
    ...
    return {
        "id": dish["id"],
        "name": dish["name_en"],
        "portion_note": dish["portion_note_en"],
        "diy_cost": round(total, 2),
        "ingredients": ingredients,
    }
...
def build_built_payload(lang: str) -> dict:
    ...
    for dish_id in load_manifest(lang):
        ...
        if lang == "en":
            built = build_diy_for_dish_en(template, load_catalog("en"))
        else:
            built = build_diy_for_dish_pl(template, load_products_seed("pl"))
        dishes.append(built)
...
def load_built(lang: str) -> dict:
    ...
    if not path.exists():
        raise FileNotFoundError(f"built file missing: {path} — run build_compare_dishes.py")
