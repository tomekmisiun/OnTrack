...
import { mealPlan as api, recipes as recipesApi, products as productsApi, fuel as fuelApi } from '../api';
...
function PieChart({ slices, size = 100, interactive = false }) {
  ...
  const cx = size/2, cy = size/2, r = size*0.42, holeR = size*0.26;
  ...
  const paths = slices.map(sl => {
    ...
    const hx1 = cx + holeR * Math.cos(angle - sweep), hy1 = cy + holeR * Math.sin(angle - sweep);
    const hx2 = cx + holeR * Math.cos(angle), hy2 = cy + holeR * Math.sin(angle);
    const large = sweep > Math.PI ? 1 : 0;
    return { ...sl, d: `M${x1},${y1} A${r},${r} 0 ${large},1 ${x2},${y2} L${hx2},${hy2} A${holeR},${holeR} 0 ${large},0 ${hx1},${hy1} Z` };
  });
    ...
    <svg width={size} height={size} style={interactive ? { cursor:'default' } : {}}>
      {paths.map((p, i) => (
        <path key={i} d={p.d}
          ...
          transform={hovered === i ? `translate(${Math.cos((paths[i]._midAngle||0))*4},${Math.sin((paths[i]._midAngle||0))*4})` : ''}
...
function SummaryCol({ lines, breakdown, tileTotal }) {
  return (
    <div style={{ borderLeft:'1px solid #1a3a38', paddingLeft:10, minWidth:100, display:'flex', flexDirection:'column', justifyContent:'center', gap:3 }}>
      {lines.map((l,i) => <div key={i} style={{ fontSize:10, color:'#9ca3af', lineHeight:1.4 }}>{l}</div>)}
      {breakdown && breakdown.length > 0 && (
        <div style={{ borderTop:'1px solid #1a3a38', paddingTop:4, marginTop:2, display:'flex', flexDirection:'column', gap:2 }}>
          {breakdown.map((b,i) => (
            <div key={i} style={{ display:'flex', justifyContent:'space-between', gap:6, fontSize:10 }}>
              <span style={{ color:'#6b7280' }}>{b.label}</span>
              <span style={{ color:'#9ca3af', fontWeight:600 }}>{b.val.toFixed(2)} zł</span>
            </div>
          ))}
        </div>
      )}
      <div style={{ fontSize:15, fontWeight:800, color:'#0d9488', marginTop:2, borderTop: breakdown?.length ? '1px solid #1a3a38' : 'none', paddingTop: breakdown?.length ? 4 : 0 }}>{tileTotal.toFixed(2)} zł</div>
    </div>
  );
}
