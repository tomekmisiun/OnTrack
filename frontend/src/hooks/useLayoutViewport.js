...
function isNarrowScreen(layoutWidth) {
  return window.innerWidth < layoutWidth;
}

/** Fixed desktop viewport on narrow screens; normal responsive width on wide monitors. */
export function useLayoutViewport(layoutWidth) {
  useEffect(() => {
    const meta = document.querySelector('meta[name="viewport"]');
    if (!meta || !layoutWidth) return undefined;

    if (!meta.dataset.defaultViewport) {
      meta.dataset.defaultViewport = meta.getAttribute('content') || DEFAULT_VIEWPORT;
    }
    ...
    const apply = () => {
      ...
      meta.setAttribute(
        'content',
        narrow ? `width=${layoutWidth}` : (meta.dataset.defaultViewport || DEFAULT_VIEWPORT),
      );
    };
    ...
    return () => {
      ...
      meta.setAttribute('content', meta.dataset.defaultViewport || DEFAULT_VIEWPORT);
    };
  }, [layoutWidth]);
}
