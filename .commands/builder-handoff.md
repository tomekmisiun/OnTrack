...
**Mode:** structured handoff only. No commit/push/merge unless user writes `approve`.

## Before handoff

1. Feature branch (not default branch unless requested).
2. Run validation per `.ai-rules/agent-orchestration.md` and `.ai-rules/validation.md`.
3. Gather: branch name, `git diff --name-status <base>...HEAD`, validation output.
...
**Branch:** `<name>` (base: `<default-branch>`)

**Summary:** <2–4 sentences: what changed, key design choice, scope boundary>

**Files changed:**
<paste `git diff --name-status <base>...HEAD` only>

**Validation:**
| Command | Result | Evidence |
|---------|--------|----------|
| <from validation.md> | PASS / FAIL / N/A | brief |

**Tests added/changed:** <paths or "None">

**Migration/API/security impact:** <one line each, or "None">
...
Reference: `docs/development/ai/workflows.md`
