...
def load_mappings():
    with open(MAPPINGS_FILE) as f:
        data = json.load(f)
    return {k: v for k, v in data.items() if k != "_comment" and k != "example"}
...
async def extract_price_from_page(page, url: str) -> float | None:
    ...
    for selector in AUCHAN_PRICE_SELECTORS:
        try:
            ...
            for el in elements:
                ...
                m = PRICE_PATTERN_SIMPLE.search(text)
                if m:
    ...
    # Metoda 2: pełny tekst strony
    body_text = await page.inner_text("body")
    matches = PRICE_PATTERN.findall(body_text)
    if matches:
        candidates = [float(m.replace(",", ".")) for m in matches if 0.1 < float(m.replace(",", ".")) < 9999]
        if candidates:
            return candidates[0]
...
async def run_scraper(dry_run: bool):
    ...
    async with async_playwright() as pw:
        ...
        for product_id, stores in mappings.items():
            ...
            name, old_price = row
            print(f"Scraping: {name} (ID={product_id})...", end=" ", flush=True)

            try:
                new_price = await extract_price_from_page(page, auchan_url)
