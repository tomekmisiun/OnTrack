import type { ParsedScheduleBlock } from "@/types/daySchedule";

export function parseTimePart(str: string): { hour: number; minute: number } | null {
...
function toStartHour({ hour }: { hour: number }): number {
  return hour;
}

function toEndHour({ hour, minute }: { hour: number; minute: number }): number {
  if (minute === 0) return hour;
  return hour + 1;
}

function matchTimeRange(text: string): RegExpMatchArray | null {
  const patterns = [
...
/** Parses e.g. "7:30 12:00 praca", "8 do 16 praca", "16:15 -17". */
export function parseScheduleBlockText(text: string): ParsedScheduleBlock | null {
...
export function hourToTimeInputValue(hour: number, minute = 0): string {
  return `${String(hour).padStart(2, "0")}:${String(minute).padStart(2, "0")}`;
}

export function parseTimeInputStart(value: string): number | null {
...
export function parseTimeInputEnd(value: string): number | null {
  if (!value) return null;
  const part = parseTimePart(value);
  if (!part) return null;
...
export function parseTimeInputPart(
  value: string,
...
export function endHourToTimeInputValue(
  endHour: number,
  endPartHour = endHour,
  endPartMinute = 0,
): string {
...
export function endHourToModalTime(endHour: number): string {
