...
import { getRange, getSummary } from "@/lib/api/mealPlan";
...
function tStr(t: TFn, key: TranslationKey): string {
  return String(t(key));
}

type ShopSummaryItem = {
  product_id: number;
  product_name: string;
  total_weight: number;
...
function exportHelpItems(t: TFn) {
  return [
...
function ExportHelpModal({ open, onClose, t }: { open: boolean; onClose: () => void; t: TFn }) {
  useEffect(() => {
    if (!open) return;
    ...
    <div className="export-help-modal-backdrop" onClick={onClose}>
      <div className="export-help-modal" onClick={e => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="export-help-title">
        <div className="export-help-modal-header">
          <h2 id="export-help-title" className="export-help-modal-title">{tStr(t, 'export_how_title')}</h2>
          <button type="button" className="export-help-modal-close" onClick={onClose} aria-label={tStr(t, 'cancel')}>×</button>
        </div>
        <div className="export-help-modal-body dark-scroll">
          <div className="export-help-grid">
            {exportHelpItems(t).map(({ label, desc }) => (
              <div key={String(label)} className="export-help-item">
                <span className="export-help-item-label">{String(label)}</span>
                <span className="export-help-item-desc">{String(desc)}</span>
              </div>
            ))}
          </div>
        </div>
...
function calcDrinks(days: number, lang: string) {
  try {
    ...
    const priceForType = (t: string) => (3/1000) * (t === 'cukier' ? cukierPrice : slodzikPrice);
    const L = lang === 'en';
...
function calcOtherSummary(days: number, lang: string) {
  try {
    const n = (v: unknown) => parseFloat(String(v)) || 0;
...
function generateMacroHTML({ gender, age, weight, height, activity, goal, macros, memberName, lang }: Record<string, any>) {
  ...
  const actLabel= L ? (ACT_LABELS_EN[String(activity) as keyof typeof ACT_LABELS_EN] || String(activity)) : (MACRO_ACTIVITY.find(x=>String(x.value)===String(activity))?.label || String(activity));
  const goalLabel= L ? (GOAL_LABELS_EN[goal as keyof typeof GOAL_LABELS_EN] || goalOpt.label) : goalOpt.label;
...
function isScheduleSleepHour(hour: number) {
...
function generateDayScheduleHTML({ blocks, memberName, lang, emptyTemplate, weekLabel }: Record<string, any>) {
  ...
  const skip = new Set();
  const blockStart: Record<string, ScheduleBlock> = {};
  if (!emptyTemplate && blocks?.length) {
    blocks.forEach((b: ScheduleBlock) => {
...
// ─── Main component ───────────────────────────────────────────────────────────
type ExportScreenProps = { onGoToTab?: (tab: string) => void };

export function ExportScreen(props: ExportScreenProps = {}) {
  const { onGoToTab: onGoToTabProp } = props;
  ...
  const handleExportMacro = () => {
    ...
    if (activeMember?.weight && activeMember?.height) {
      formData = { gender: activeMember.gender ?? 'm', age: activeMember.age ?? 0, weight: activeMember.weight, height: activeMember.height, activity: activeMember.activity ?? 1.55, goal: activeMember.goal ?? 'maintain' };
    ...
    if (!formData || !(parseFloat(formData.weight)>0 && parseFloat(formData.height)>0 && parseInt(formData.age)>0)) {
      showError(tStr(t, 'export_macro_err'));
      return;
    }
    if (!macros) {
    ...
    if (!html) { showError(tStr(t, 'export_macro_gen_err')); return; }
    openHtmlWindow(html);
  };
  ...
  const [htmlPeriod, setHtmlPeriod] = useState<'week' | 'month' | 'custom'>('week');
  const [htmlCustom, setHtmlCustom] = useState<{ start: string; end: string }>({ start: '', end: '' });
  ...
  const [shopDays, setShopDays] = useState<Set<string>>(() => new Set());
  ...
  useEffect(() => { listRecipes().then(setRecipes).catch(() => {}); }, [lang]);

  const [previewMeals, setPreviewMeals] = useState<MealsByDate>({});
  ...
  useEffect(() => {
    ...
    getRange(range.start, range.end, ids).then((data) => { setPreviewMeals(data || {}); setPreviewLoading(false); }).catch(() => setPreviewLoading(false));
  ...
  const toggleShopMember = (id: number) => {
    if (id === activeMember?.id) return;
  ...
  const selectShopMonth = () => {
    const s = new Set<string>(getCalGrid(shopYear, shopMonth).filter(d => d.getMonth() === shopMonth).map(dateToStr));
    setShopDays(s);
  };
  const handleGenShopList = async () => {
    if (shopDays.size === 0) { showError(tStr(t, 'select_at_least_one_day')); return; }
    setShopLoading(true);
    try {
      const days = [...shopDays].sort();
      const results = await Promise.all(days.map((d) => getSummary(d, d, shopMids)));
      const merged: Record<number, ShopSummaryItem> = {};
      for (const res of results) {
        for (const item of res.items as ShopSummaryItem[]) {
          const existing = merged[item.product_id];
          if (!existing) {
            merged[item.product_id] = { ...item, total_weight: 0 };
          }
  ...
  const getHtmlPeriodLabel = () => {
    if (htmlPeriod === 'week')  return `${tStr(t, 'this_week')} (${toEU(week.start)} – ${toEU(week.end)})`;
    if (htmlPeriod === 'month') return `${tStr(t, 'this_month')} (${toEU(month.start)} – ${toEU(month.end)})`;
    const r = htmlCustom;
  ...
  const handleGenWydatki = async () => {
    ...
    if (!range.start || !range.end) { showError(tStr(t, 'export_select_period')); return; }
    setHtmlWLoading(true);
    try {
      const d = await getSummary(range.start, range.end, mids);
      const days = Math.round((new Date(range.end).getTime() - new Date(range.start).getTime()) / 86400000) + 1;
      const categories: { label: string; value: number }[] = [{ label: tStr(t, 'food_label'), value: d.total_cost }];
      const drinkItems = calcDrinks(days, lang);
      const drinkTotal = drinkItems.reduce((s, i) => s + i.total, 0);
      if (drinkTotal > 0) categories.push({ label: tStr(t, 'drinks_label'), value: drinkTotal });
      calcOtherSummary(days, lang).forEach((item) => {
  ...
  const handleGenKalendar = async () => {
    ...
    try {
      const mealsByDate = await getRange(range.start, range.end, mids);
      const html = generateKalendarzHTML({ mealsByDate, start: range.start, end: range.end, periodLabel: getHtmlPeriodLabel(), memberName: memberLabel, lang });
      openHtmlWindow(html);
    } catch { showError(tStr(t, 'export_calendar_err')); }
    finally { setHtmlKLoading(false); }
  };

  const handleGenSchedule = async (emptyTemplate: boolean) => {
    if (!activeMember?.id) { showError(tStr(t, 'export_schedule_err')); return; }
    setHtmlScheduleLoading(true);
    try {
      let blocks: ScheduleBlock[] = [];
      const { start, end } = getCurrentWeek();
      if (!emptyTemplate) {
        blocks = await getScheduleAll(activeMember.id, start);
      }
      ...
      openHtmlWindow(html);
    } catch { showError(tStr(t, 'export_schedule_err')); }
    finally { setHtmlScheduleLoading(false); }
  };

  const handleGenSkladniki = async () => {
  ...
  const LBL: CSSProperties = { fontSize:11, color:'#6b7280', marginBottom:8, textTransform:'uppercase', letterSpacing:'0.5px', fontWeight:700 };
    ...
    <div>
      <div className="card" style={{ padding:'16px 20px', marginBottom:12 }}>
        ...
        <div className="export-section-head">
          <h2 className="card-section-title">{tStr(t, 'export_generate_title')}</h2>
          <button
            type="button"
            className="pill-help-btn"
            onClick={() => setExportHelpModalOpen(true)}
            aria-label={tStr(t, 'export_how_title')}
            title={tStr(t, 'export_how_title')}
          >
            <Icon icon="heroicons:light-bulb" width={15} />
            <span>{tStr(t, 'import_help_btn')}</span>
          </button>
        </div>

        <div style={{ marginBottom:12 }}>
          <div style={LBL}>{tStr(t, 'export_period_label')}</div>
          <div style={{ display:'flex', gap:8, flexWrap:'wrap' }}>
            <ToggleChip active={htmlPeriod==='week'}   onClick={()=>setHtmlPeriod('week')}>{tStr(t, 'this_week')}</ToggleChip>
            <ToggleChip active={htmlPeriod==='month'}  onClick={()=>setHtmlPeriod('month')}>{tStr(t, 'this_month')}</ToggleChip>
            <ToggleChip active={htmlPeriod==='custom'} onClick={()=>setHtmlPeriod('custom')}>{tStr(t, 'custom_period')}</ToggleChip>
          </div>
          {htmlPeriod === 'custom' && (
            <div style={{ marginTop:10, display:'flex', gap:12, flexWrap:'wrap' }}>
              {([{ key: 'start' as const, label: tStr(t, 'date_from') }, { key: 'end' as const, label: tStr(t, 'date_to') }]).map(({ key, label }) => (
                <div key={key}>
                  <div style={{ fontSize:11, color:'#6b7280', marginBottom:3 }}>{label}</div>
                  <div style={{ position:'relative' }}>
                    <input type="text" readOnly placeholder={tStr(t, 'date_format_ph')}
        ...
        {members.length > 1 && (
          <div style={{ marginBottom:14 }}>
            <div style={LBL}>{tStr(t, 'export_who_label')}</div>
          ...
          <div style={{ position:'relative' }}>
            ...
            {skladnikiOpen && (
              <div style={{
                ...
                <input type="text" placeholder={tStr(t, 'export_recipe_search')} autoFocus
                ...
                {skladnikiSearch.trim() && (
                  <div style={{ maxHeight:200, overflowY:'auto', border:'1px solid #374151', borderRadius:8, marginBottom:10 }}>
                    ...
                    {recipes.filter(r => fuzzySearch(skladnikiSearch, r.name)).length === 0 && (
                      <div style={{ padding:'12px', fontSize:12, color:'#6b7280', textAlign:'center' }}>{tStr(t, 'export_no_results')}</div>
                    )}
                  </div>
                )}
        ...
        <div style={{ flexShrink:0, width:280 }}>
          <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6 }}>
            <h2 className="card-section-title" style={{ margin: 0 }}>{tStr(t, 'shopping_list_title')}</h2>
            <button onClick={() => goToTab('calendar')}
          ...
          <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:8 }}>
            ...
            <span style={{ flex:1, textAlign:'center', fontWeight:700, fontSize:13, color:'#e2e8f0' }}>
              {tArray(t, 'month_names')[shopMonth]} {shopYear}
            </span>
          ...
          <div style={{ display:'flex', gap:5, flexWrap:'wrap', marginBottom:8 }}>
            <ToggleChip active={false} onClick={selectShopWeek}>{tStr(t, 'this_week')}</ToggleChip>
            <ToggleChip active={false} onClick={selectShopMonth}>{tStr(t, 'this_month')}</ToggleChip>
            {shopDays.size > 0 && <ToggleChip active={false} onClick={() => setShopDays(new Set())}>{tStr(t, 'clear')}</ToggleChip>}
          </div>

          {shopDays.size > 0 && (
            <div style={{ fontSize:11, color:'#0d9488', fontWeight:600, marginBottom:8 }}>
              {tFormatN(t, 'selected_days', shopDays.size)}
            </div>
          )}

          {members.length > 1 && (
            <div style={{ marginBottom:10 }}>
              <div style={{ ...LBL, fontSize:10, marginBottom:6 }}>{tStr(t, 'export_include_profiles')}</div>
          ...
          <button className="btn btn-primary" onClick={handleGenShopList}
            ...
            {shopLoading ? tStr(t, 'generating') : `${tStr(t, 'export_generate_list')}${shopDays.size > 0 ? ` (${shopDays.size})` : ''}`}
          </button>
        </div>
        ...
        {(() => {
          ...
          const daysWithMeals = allDays.filter((d) => (previewMeals[dateToStr(d)] || []).length > 0);

          return (
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:8 }}>
                <span style={{ fontSize:13, fontWeight:700, color:'#e2e8f0' }}>{tStr(t, 'export_preview_title')}</span>
                <span style={{ fontSize:11, color:'#6b7280' }}>{previewLabel}</span>
              </div>
              {previewLoading ? (
                <div style={{ fontSize:12, color:'#6b7280', textAlign:'center', padding:'20px 0' }}>{tStr(t, 'loading_preview')}</div>
              ) : (
                <>
                  ...
                  <div style={{ fontSize:11, color: totalMeals > 0 ? '#6b7280' : '#374151', textAlign:'center', marginTop:2 }}>
                    {totalMeals === 0
                      ? tStr(t, 'export_no_meals')
                      : tFormat2(t, 'days_with_meals', daysWithMeals.length, totalMeals)}
                  </div>
                </>
              )}
            </div>
