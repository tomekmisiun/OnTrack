...
Usage (via Docker):
    docker exec mealprep-app-1 python /app/scraper/pipeline/import_to_db.py --user-id 2
    docker exec mealprep-app-1 python /app/scraper/pipeline/import_to_db.py --user-id 2 --lang pl
    docker exec mealprep-app-1 python /app/scraper/pipeline/import_to_db.py --list-users
    docker exec mealprep-app-1 python /app/scraper/pipeline/import_to_db.py --user-id 2 --clear
"""
...
def _data_path(filename: str) -> Path:
    return _DATA_FILES.get(filename, SCRAPER_ROOT / "data" / filename)
...
def repair_ingredient_matches(ingredients: list[dict], lang: str) -> list[dict]:
    ...
    shops_file = SHOPS_EN if lang == "en" else SHOPS_PL
    if not shops_file.exists():
        return ingredients

    sys.path.insert(0, str(Path(__file__).parent.parent))
    from pipeline.match_ingredients import rank_candidates
