...
from app.services.catalog_seed_service import _load_json
...
def test_validate_runtime_data_fails_on_empty_product_name(tmp_path):
    ...
    bad = json.loads((root / "seeds/products_seed_pl.json").read_text())
    bad[0]["name"] = ""
    (root / "seeds/products_seed_pl.json").write_text(json.dumps(bad), encoding="utf-8")
...
def test_runtime_data_dir_points_at_backend_data(use_backend_data):
    ...
    assert seeds_dir() == BACKEND_DATA / "seeds"
...
def test_import_names_uses_backend_data_macros(use_backend_data):
    # demo macro map: PL name may fuzzy-match to EN label
...
def test_catalog_seed_loads_backend_data_products(use_backend_data):
    products = _load_json("products_seed_pl.json", "pl")
